...
def latent2text(
    model_vae, decoder_tokenizer, z, print_text=False, logits=None, word="dog"
):
    device = z.device
...
def sample_q_ode(
    ...
    **kwargs
    ...
    # ODE function
    vpode = VPODE(
        ccf, y, save_path=save_path, plot=plot, every_n_plot=every_n_plot, kwargs=kwargs
    )
    states = (z_k,)
    integration_times = torch.linspace(vpode.T, 0.0, 2).type(torch.float32).to(device)
...
def main():
    ...
    parser.add_argument(
        "--checkpoint_dir",
        default="/data/taoshuchang/experiments/ODE_Sampling/model/imdb_agnews_gap/2023-06-16 21:34:46_seed103_bert_gpt2_fix84_lr8e-05_latent128_bs64_epoch50_beta0.9_dim_target_kl0.9_wl1_wg0.0",
        type=str,
    ...
    parser.add_argument(
        "--output_dir",
        default="/data/taoshuchang/experiments/ODE_Sampling/output/imdb_agnews_gap/",
        type=str,
    ...
    parser.add_argument(
        ...
        default="/data/taoshuchang/experiments/gpt2-medium",
        type=str,
    ...
    parser.add_argument("--cls_step", type=str, default="1,2")
    parser.add_argument("--att_val_list", type=str, default="1,0")
    parser.add_argument("--data_type", type=str, default="yelp")
    ...
    args.use_pretrained_vae = True

    args.gan_dir = os.path.join(args.checkpoint_dir, "checkpoint-gan-1")
    args.cls_dir = os.path.join(args.checkpoint_dir, "checkpoint-cls-")
    args.output_dir = args.checkpoint_dir.replace("/model/", "/outputs/")
    # args.output_dir = "/data/taoshuchang/experiments/ODE_Sampling/outputs/random"
    ...
    for cls_list in pbar:
        ...
        for ii, cls_ in enumerate(cls_step_list):
            if cls_.isdigit():
                ...
                num_classes = 2
            classifier_ = DenseEmbedder(
                args.latent_size, up_dim=2, depth=4, num_classes=num_classes
            )
            cls_dir = os.path.join(args.cls_dir + cls_, "training_cls.bin")
            cls_checkpoint = torch.load(cls_dir)
            classifier_.load_state_dict(
                cls_checkpoint["model_state_dict"], strict=False
            )
            if weight_dict != None:
                classifier_.set_energy_weight(weight_dict[str(cls_)])
            else:
        ...
        mode = [1, 0, 0]
        condSampling = ConditionalSampling(
            sample_q,
        ...
        # 写个循环一次跑完
        for senti_idx in range(2):
            for topic_idx in range(4):
                att_val_list = ",".join([str(senti_idx), str(topic_idx)])
                condSampling.get_samples_multiple(att_val_list.split(";"), mode=mode)
